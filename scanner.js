...
const SCRAPER_API_PROXY_PASS = "724e556461bcfc9e5528b4c56d5c30d6";
...
export const getDomFeatures = async (page, url, userAgent) => {
  ...
  const inlineJS = await safeEval(
    () => {
      ...
      document.querySelectorAll("*").forEach((el) => {
        handlers += [...el.attributes].filter((a) =>
          a.name.startsWith("on")
        ).length;
        if (typeof el.href === "string" && el.href.startsWith("javascript:"))
          hrefJS++;
      });
      return {
  ...
  const loginForms = await safeEval(
    () => {
      ...
      [...forms].forEach((form) => {
        const hasPassword = [...form.elements].some(
          (el) => el.type === "password"
        );
        const rawAction = form.action || "";
        const actionStr =
          typeof rawAction === "string" ? rawAction : String(rawAction);
        const isExternal = actionStr && !actionStr.includes(location.hostname);
        const isBinary = suspiciousExtensions.some((ext) =>
          actionStr.endsWith(ext)
        );

        if (hasPassword) loginCount++;
        if (hasPassword && isExternal && isBinary) externalFormActionBinary++;
        if (hasPassword && isExternal && !isBinary)
  ...
  const suspiciousScripts = await safeEval(
    () => {
      ...
      document.querySelectorAll("script").forEach((script) => {
        if (suspiciousSrc.some((word) => (script.src || "").includes(word)))
          suspicious++;
      });
  ...
  const otherFeatures = await safeEval(
    () => {
      ...
      const externalLinks = [...document.querySelectorAll("a")].filter(
        (a) => typeof a.href === "string" && !a.href.includes(location.hostname)
      ).length;
      const embeddedObjects = document.querySelectorAll("embed, object").length;
      const suspiciousInlineStyles = [...document.querySelectorAll("*")].filter(
        (el) =>
          /display\s*:\s*none|visibility\s*:\s*hidden/i.test(
            el.getAttribute("style") || ""
          )
      ).length;
      const suspiciousLinkCount = [...document.querySelectorAll("a")].filter(
        (a) => typeof a.href === "string" && /\.php|\.exe|base64/i.test(a.href)
      ).length;
    ...
    () => {
      ...
      for (let i = 0; i < window.frames.length; i++) {
        try {
          ...
          if (frameUrl && !frameUrl.includes(location.hostname))
            externalFrameCount++;
          if (
            frameUrl &&
            suspiciousIndicators.some((ind) => frameUrl.includes(ind))
          ) {
...
export const scanUrls = async (urls, concurrencyLimit = 10) => {
    ...
    for (let { url, sslBypass } of versions) {
      ...
      try {
        ...
        if (err.message.includes("Protocol error: Connection closed.")) {
          console.warn(
            `🔁 Relaunching browser due to closed connection at ${url}`
          );
          await delay(3000);
          await launchBrowser();
        }

        if (proxyRetryErrors.some((e) => err.message.includes(e)))
          proxyNeeded = true;
      } finally {
    ...
    if (proxyNeeded) {
      ...
      await proxyLimit(async () => {
        for (let { url } of versions) {
          let page;
          try {
            const scraperApiUrl = `http://api.scraperapi.com/?api_key=${SCRAPER_API_PROXY_PASS}&url=${encodeURIComponent(
              url
            )}`;
  ...
  const tasks = urls.map(({ url, label }) =>
    limit(async () => {
      try {
        const { features, finalUrl, sslBypassUsed, usedProxy, userAgent } =
          await scanWithRetry(url);
        results[url] = {
          ...features,
          sslBypassUsed,
